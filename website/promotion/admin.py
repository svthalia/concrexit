...
class PromotionRequestAdmin(admin.ModelAdmin):
    ...
    list_display = ("event", "publish_date", "channel", "assigned_to", "status", "created_at")
    list_filter = (
        "publish_date",
        "assigned_to",
        "status",
    ...
    def mark_not_started(self, request, queryset):
        """Change the status of the request to published."""
        self._change_status(queryset, PromotionRequest.NOT_STARTED)

    mark_not_started.short_description = ("Mark requests as not started")

    def mark_started(self, request, queryset):
        """Change the status of the request to published."""
        self._change_status(queryset, PromotionRequest.STARTED)

    mark_started.short_description = ("Mark requests as started")

    def mark_finished(self, request, queryset):
        """Change the status of the request to published."""
        self._change_status(queryset, PromotionRequest.FINISHED)

    mark_finished.short_description = ("Mark requests as finished")
    
    def mark_published(self, request, queryset):
        """Change the status of the request to published."""
        self._change_status(queryset, PromotionRequest.PUBLISHED)
