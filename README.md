...
1. Install [`uv`](https://docs.astral.sh/uv/getting-started/installation/) and [Pillow dependencies](#pillow-dependencies) as per below.
2. Open the code in your favorite Python IDE (we have some helpful default settings provided for VSCode).
3. `uv run website/manage.py migrate` to set up the database.
4. `uv run website/manage.py createcachetable` to set up a cache.
5. `uv run website/manage.py createsuperuser` to create an admin user.
6. `uv run website/manage.py createfixtures -a` to generate a bunch of test data.
7. `uv run website/manage.py runserver` to run a testing server. Now you are able to visit your local concrexit at http://127.0.0.1:8000
...
- `git add .` to add all your changes to a commit
